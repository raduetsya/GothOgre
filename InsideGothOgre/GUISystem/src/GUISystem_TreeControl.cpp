...
namespace GUISystem
{
	...
	TreeControl::Node::Node(const MyGUI::UString& strText, Node* pParent) :
	...
	TreeControl::Node::Node(const MyGUI::UString& strText, const MyGUI::UString& strImage, Node* pParent) :
		GenericNode<Node, TreeControl>(strText, pParent),
		mbIsPrepared(false),
		mbIsExpanded(false),
		mstrImage(strImage)
	...
	void TreeControl::Node::setImage(const MyGUI::UString& strImage)
	{
		mstrImage = strImage;
	...
	TreeControl::~TreeControl()
	{
		if(mbInvalidated && MyGUI::Gui::getInstancePtr())
		{
			MyGUI::Gui::getInstance().eventFrameStart -= newDelegate(this, &TreeControl::notifyFrameEntered);
		}
	}
	...
	void TreeControl::initialiseOverride()
	{
		...
		if (isUserString("HeightLine"))
			mnItemHeight = MyGUI::utility::parseValue<int>(getUserString("HeightLine"));
		if (isUserString("LevelOffset"))
			mnLevelOffset = MyGUI::utility::parseValue<int>(getUserString("LevelOffset"));
	...
	void TreeControl::notifyMousePressed(MyGUI::Widget* pSender, int nLeft, int nTop, MyGUI::MouseButton nID)
	{
	...
	void TreeControl::notifyMouseWheel(MyGUI::Widget* pSender, int nValue)
	{
		if (mnScrollRange <= 0)
			return;
	...
	void TreeControl::notifyMouseDoubleClick(MyGUI::Widget* pSender)
	{
		if (mpSelection)
			eventTreeNodeActivated(this, mpSelection);
	}

	void TreeControl::notifyMouseSetFocus(MyGUI::Widget* pSender, MyGUI::Widget* pPreviousWidget)
	{
		mnFocusIndex = *pSender->_getInternalData<size_t>();
		eventTreeNodeMouseSetFocus(this, mItemWidgets[mnFocusIndex]->getNode());
	}

	void TreeControl::notifyMouseLostFocus(MyGUI::Widget* pSender, MyGUI::Widget* pNextWidget)
	{
	...
	void TreeControl::notifyScrollChangePosition(MyGUI::ScrollBar* pSender, size_t nPosition)
	{
		scrollTo(nPosition);
		sendScrollingEvents(nPosition);
	}

	void TreeControl::notifyExpandCollapse(MyGUI::Widget* pSender)
	{
		TreeControlItem* pItem = pSender->getParent()->getParent()->castType<TreeControlItem>(false); // The second "->getParent()" has been added by Kerrax.
		if (!pItem)
			return;
